...
<body>
    ...
    <div id="container">
        ...
        <div class="row">
            <h4 class="classic-title"><span>All <?php echo $_SESSION["Id"];?> Requests</span></h4>
    <?php
	$query = "select * from tenacyrequest where Email='$email' and RequestId='$requestId'";
    ...
    <tr>	
	 ...
	 <?php
	  ...
	  ?>
        ...
        <div class="container" style="margin-top:30px;">
        <div class="row">
            <h4 class="classic-title"><span>Responses received</span></h4>
            <?php include("db/opendb.php"); 
	$query = "select * from response where RequestId = '$requestId'";
	$result = $con -> query($query) or die("Query error");
	?> 
	<table class="table">
  <thead class="thead-inverse">
  ...
  <tbody>
   <?php 
    ...
    <!--<td><?php echo $row['RequestId']; ?></td>
	  ...
	  <td><a href="https://www.google.com.sg/maps/place/Singapore+<?php echo $row['Pincode']; ?>" target="_blank">View </a>
	  </td>
	  <td><a href="<?php echo $row['Url']; ?>" target="_blank"><?php echo $row['Url']; ?></a></td>
	  <td><?php if($row['PriceNegotiable']=='1'){ echo "Yes";} else{echo "No";}  ?></td>
      <td><?php if($row['Status']=="pending"){ ?><a href="accept.php?id=<?php echo $row['ResponseId']; ?>" name="accept" class="btn btn-success btn-sx">Accept</a><?php } else if($row['Status']=="accepted"){ echo "Responses received";}?></td>
      <td><a href="viewAgentDetails.php?id=<?php echo $row['Email']; ?>" name="accept" class="">
	  View Agent Details</a></td>
    </tr>
	 <?php
	  }
	  ?>
